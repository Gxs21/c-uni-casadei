...
int check_pattern(char *s, char *template) {
    int i = 0; // cursor on s
    int j = 0; // cursor on template
    int star = -1; // position of last star in template
    int match = 0; // position in s which corresponds to the star in template
    // Loop until we reach the end of s
    while (s[i]) {
        if (template[j] == '?' || template[j] == s[i]) {
            i++;
            j++;
        } else if (template[j] == '*') {
            star = j;
    ...
    return template[j] == '\0'; // true if we reached the end of template
}
