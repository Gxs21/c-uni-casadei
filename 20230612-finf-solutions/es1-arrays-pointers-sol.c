...
/*
Realizzare una funzione `f` che accetta un array di interi e restituisce un nuovo array SENZA DUPLICATI
 (ovvero mantenendo solo LA PRIMA OCCORRENZA di ogni intero contenuto).
Ad esempio, dato `f(x,4,ptr)` con `x` che punta all'array di lunghezza 4 con contenuto `[1,2,1,4]`, 
 il risultato dev'essere l'intero 3 corrispondente alla lunghezza del nuovo array, 
 e il parametro puntatore `ptr` va fatto puntare alla memoria con il contenuto `[1,2,4]` (dove 
 il secondo `1` dell'array in input è stato tralasciato).
Si osservi la funzione `test` sotto riportata per ulteriori esempi.

ATTENZIONE: si noti che il terzo parametro di `f` è un "puntatore di puntatore": 
 è dichiarato infatti come `int** result`. Il modo corretto di assegnarlo è come segue:

    *result = (int *) malloc ( ... );

oppure anche:

    int *my_output = (int *) malloc ( ... );
    // ... altro codice ...
    *result = my_output;
...
int f(int* a, int length, int** result) {
...
int check(int* input, int input_len, int* expected, int expected_len) {
    int *actual = NULL;
    int actual_len = f(input, input_len, &actual);
