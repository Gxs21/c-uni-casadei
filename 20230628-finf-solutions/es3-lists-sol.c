...
list* no_duplicates(list *A, int k) {
    ...
    while(R != NULL) {
        int n_occ = 1;
        ptr = R->next;
        list *prev = R; // pointer to the last non-removed element while we are iterating
        while(ptr != NULL) {
            if(ptr->val == R->val) n_occ++;
            if(ptr->val == R->val && n_occ > k) {
                ...
                // no need to remove, just move ptr ahead
                prev = ptr;
                ptr = ptr->next;
            }
        }
