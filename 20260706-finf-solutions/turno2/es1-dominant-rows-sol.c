...
Ci si attende quindi che la funzione scriva in `result` l'array:
...
int sum_others(int *row, int n, int exclude){
    int sum = 0;
    for(int i=0; i < n; i++){
        if(i != exclude){
            sum += row[i];
...
int check_dominance(int *row, int n){
    for(int i=0; i < n; i++){
        int sum = sum_others(row, n, i);
        if(row[i] > sum){
            return 1;
        }
    }
