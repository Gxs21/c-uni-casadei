...
int** f(int N, int K) {
    ...
    int lim_left = -1, lim_bottom = N, lim_right = N, lim_top = -1; 
    int i = 0, j = 0;
    int dir = down;
    while(j > lim_left && j < lim_right && i < lim_bottom && i > lim_top) {
        ...
        result[i][j] = K++;
        if(dir == down) {
            if(i+1 >= lim_bottom) { dir = right; j++; lim_left++; printf("{turn right}"); } else { i++; }
        }
        else if(dir == up) {
            if(i-1 <= lim_top) { dir = left; j--; lim_right--; printf("{turn left}"); } else { i--; }
        }
        else if(dir == left) {
            if(j-1 <= lim_left) { dir = down; i++; lim_top++; printf("{turn down}"); } else { j--; }
        }
        else if(dir == right) {
            if(j+1 >= lim_right) { dir = up; i--; lim_bottom--; printf("{turn up}"); } else { j++; }
        }
    }
...
void test(){
    ...
    check("5x5 spiral starting at K=0", 5, 0, build_matrix(5, 5, (int[]) {
       0, 15, 14, 13, 12,
       1, 16, 23, 22, 11,
       2, 17, 24, 21, 10,
       3, 18, 19, 20, 9,
